...
find_idle_thread:
    ...
    if (CONSUMER_THREAD_NUM == i)
    {
        ...
        pthread_cond_wait(&cond_consumer_not_full, &(tbfifo[arranged_thread].mutex));
...
void multi_thread_init()
{
    ...
    for (i=0; i < CONSUMER_THREAD_NUM; i++)
    {
        ret = pthread_create(&(tbfifo.tid[i]), NULL, consumer_thread, (void*)&tbfifo.thread_index[i]);
        if (0 != ret)
        {
            _ERR("thread create");
        }
